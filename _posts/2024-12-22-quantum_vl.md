...
- 확률 모델의 부재
   - 기본 링크 생성 확률 \( P_e \)와 메모리 효율 \( \eta_m \) (따라서 얽힘 교환 성공 확률 \( P_s \))을 정확히 나타내는 모델이 존재하지 않음
   - 상태 전이 확률 \( p(s_t, a_t, s_{t+1}) \)도 알 수 없음

- 깊은 강화학습(DRL)의 적용
   - 가상 링크 생성률을 극대화하는 정책을 찾기 위해 DRL 적용
   - 이 과정에서 \( Q \)-값 함수 \( Q^\pi(s, a) \)를 정의하여 사용

- \( Q \)-값 함수 정의
   - \( Q^\pi(s, a) \): 주어진 상태-행동 쌍 \( (s, a) \)에 대해 정책 \( \pi \)를 따랐을 때 기대되는 할인된 보상(expected discounted return)을 의미
   - 할인된 보상은 궤적(trajectory)에서 미래 보상의 합으로 정의됨:
     \[
     \sum_{k=0}^\infty \gamma^k r_{t+k+1}, \quad \gamma \in [0, 1]
     \]
   - 에이전트는 이 \( Q \)-값 함수를 극대화하는 최적 정책 \( \pi^* \)를 탐색

- 정책의 구성
   - 본 문제에서 정책은 가장 적절한 컷오프 시간 \( t_c \)를 결정하는 것을 포함
   - \( t_c \): 가장 오래된 기본 링크에 대해 컷오프 시점을 최적으로 설정

- DQN 알고리즘을 활용한 학습
   - 상태 전이 확률이 알려지지 않은 상황에서 \( Q \)-값 함수는 통계적 학습을 통해 추정
   - 본 연구에서는 **Deep Q-Network(DQN)** 알고리즘을 기반으로 가상 링크 생성 문제에 적합하도록 학습 루틴을 구성
